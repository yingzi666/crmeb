...
class AuthController
{
    ...
    public function verify(Request $request)
    {
        ...
        if (User::checkPhone($phone) && $type == 'register') {
            return app('json')->fail('手机号已注册');
        }

        if (!User::checkPhone($phone) && $type == 'login') {
    ...
    public function binding_phone(Request $request)
    {
        ...
        if ($userInfo->phone) {
            return app('json')->fail('您的账号已经绑定过手机号码！');
        }
        ...
        if (User::where('account', $phone)->where('phone', $phone)->where('user_type', 'h5')->find()) {
            if (!$step) {
                return app('json')->success('H5已有账号是否绑定此账号上', ['is_bind' => 1]);
            }
