...
use app\models\store\StoreCart;use crmeb\basic\BaseModel;
use crmeb\services\PHPExcelService;
use crmeb\services\WechatTemplateService;
use crmeb\traits\ModelTrait;use think\facade\Route as Url;
    ...
    public static function OrderList($where)
    {
        ...
        foreach ($data as &$item) {
            ...
            foreach ($_info as $k => $v) {
                ...
                if (!isset($cart_info['productInfo'])) {
                    $cart_info['productInfo'] = [];
                }
...
HTML;
    ...
    public static function updateOffline($id)
    {
        ...
        if (!$count) {
            return self::setErrorInfo('订单已支付');
        }
...
//        $model = $model->where('combination_id',0);
        ...
        if (isset($where['is_del']) && $where['is_del'] != '' && $where['is_del'] != -1) {
...
//                    $model = $model->where($aler.'combination_id',">",0)->where($aler.'pink_id',">",0);
    ...
    public static function getOrderWherePink($where, $model)
    {
        $model = $model->where('combination_id', '>', 0);
        if ($where['status'] != '') {
            $model = $model::statusByWhere($where['status']);
        }
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    public static function orderPostageAfter($oid, $postageData = [])
    {
        ...
        if ($postageData['delivery_type'] == 'send') {
//送货
...
//公众号
        ...
        } else if ($postageData['delivery_type'] == 'express') {
//发货
            if ($order['is_channel'] == 1) {
                //小程序发货模版消息
...
//小程序
    ...
    public static function integralBack($id)
    {
        ...
        if ($order['coupon_id']) {
            $res4 = StoreCouponUser::recoverCoupon($order['coupon_id']);
        }
