...
use app\admin\model\ump\StoreCombination;use app\admin\model\ump\StoreSeckill;
use crmeb\services\FormBuilder as Form;
use crmeb\services\JsonService;
use crmeb\services\JsonService as Json;
use crmeb\services\UtilService as Util;
use crmeb\traits\CurdControllerTrait;use think\facade\Route as Url;
...
class StoreProduct extends AuthController
{
    ...
    public function save($id)
    {
        ...
        if (count($data['cate_id']) < 1) {
            return Json::fail('请选择产品分类');
        }

        $cate_id         = $data['cate_id'];
        $data['cate_id'] = implode(',', $data['cate_id']);
        if (!$data['store_name']) {
            return Json::fail('请输入产品名称');
        }

        if (count($data['image']) < 1) {
            return Json::fail('请上传产品图片');
        }

        if (count($data['slider_image']) < 1) {
            return Json::fail('请上传产品轮播图');
        }
    ...
    public function update($id)
    {
        ...
        if ($data['price'] == '' || $data['price'] < 0) {
            return Json::fail('请输入产品售价');
        }

        if ($data['ot_price'] == '' || $data['ot_price'] < 0) {
            return Json::fail('请输入产品市场价');
        }

        if ($data['stock'] == '' || $data['stock'] < 0) {
            return Json::fail('请输入库存');
        }
