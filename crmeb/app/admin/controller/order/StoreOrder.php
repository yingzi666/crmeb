...
use app\admin\model\user\UserBill;use crmeb\basic\BaseModel;use crmeb\repositories\OrderRepository;use crmeb\repositories\ShortLetterRepositories;
...
//        $f[] = Form::radio('status','状态',$product->getData('status'))->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]]);
    ...
    public function update($id)
    {
        ...
        if ($data['total_price'] <= 0) {
            return Json::fail('请输入商品总价');
        }

        if ($data['pay_price'] <= 0) {
            return Json::fail('请输入实际支付金额');
        }
        ...
        switch ((int) $data['type']) {
            case 1:
                ...
                if (!$data['delivery_name']) {
                    return Json::fail('请选择快递公司');
                }

                if (!$data['delivery_id']) {
                    return Json::fail('请输入快递单号');
                }
            ...
            case 2:
                ...
                if (!$data['delivery_name']) {
                    return Json::fail('请输入送货人姓名');
                }

                if (!(int) $data['delivery_id']) {
                    return Json::fail('请输入送货人电话号码');
                } else if (!preg_match("/^1[3456789]{1}\d{9}$/", $data['delivery_id'])) {
                    return Json::fail('请输入正确的送货人电话号码');
                }
    ...
    public function updateRefundY($id)
    {
        ...
        if ($product['pay_price'] == $product['refund_price']) {
            return Json::fail('已退完支付金额!不能再退款了');
        }

        if (!$data['refund_price']) {
            return Json::fail('请输入退款金额');
        }
        ...
        if ($bj < 0) {
            return Json::fail('退款金额大于支付金额，请修改退款金额');
        }
...
//TODO 公众号
        ...
        if ($resEdit) {
            $data['type'] = $type;
            if ($data['type'] == 1) {
                $res = StorePink::setRefundPink($id);
            }
    ...
    public function updateIntegralBack($id)
    {
        ...
        if ($bj < 0) {
            return Json::fail('退积分大于支付积分，请修改退积分');
        }
